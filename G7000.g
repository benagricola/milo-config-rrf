...
;
; USAGE: "G7000 P<period> V<variance>"
...
if { param.P < global.daemonUpdateRate }
    { abort "Period cannot be less than daemonUpdateRate (" ^ global.daemonUpdateRate ^ ")" }

if { mod(param.P, global.daemonUpdateRate) > 0 }
    { abort "Period must be a multiple of daemonUpdateRate (" ^ global.daemonUpdateRate ^ ")" }
