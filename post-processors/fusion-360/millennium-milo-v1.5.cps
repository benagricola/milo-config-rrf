...
longDescription = "Millennium Machines Milo v1.5 Post Processor.";
...
var properties = {
  ...
  probeWorkPieceBeforeStart: true,
  probeWorkPieceOnWCSChange: true,
  waitForSpindle: 20,
...
var propertyDefinitions = {
  ...
  probeWorkPieceBeforeStart: {
    title: "Probe workpiece origin before start",
    description: "When enabled, machine will execute G6001 to probe a top corner prior to executing any operations.",
    group: "probing",
    scope: "machine",
    type: "boolean",
  },
  probeWorkPieceOnWCSChange: {
    title: "Probe workpiece when changing WCS",
    description: "When enabled, machine will execute G6001 to probe a top corner and set WCS Zero prior to changing into that WCS.",
    group: "probing",
    scope: "operation",
    type: "boolean",
  },
...
var G_HOME         = 28;
var G_PROBE_TOOL   = 37;
var G_PROBE_CORNER = 6001;
var G_PROBE_REF    = 6013;
...
var gCodesF = createModalGroup(
  ...
  [
      ...
      [G_PROBE_TOOL, G_PROBE_CORNER, G_PROBE_REF] // Probe codes
  ],
  gFmt);
...
function onOpen() {
  ...
  if (nTools > 0) {
    ...
    writeBlock(gCodesF.format(G_PROBE_REF));
    writeln("");
  }
...
function onSection() {
  ...
  if(wcsChanging) {
    // Check if probing required. If this is first section, only probe if
    // probeWorkPieceBeforeStart is set. Otherwise, only probe if
    // probeWorkPieceOnWCSChange is set
    var doProbe = (isFirstSection() && properties.probeWorkPieceBeforeStart) || properties.probeWorkPieceOnWCSChange;
    var wcsO = { wcs: curWCS };
    ...
    if(doProbe) {
      ...
      writeBlock(gCodesF.format(G_PROBE_CORNER), "W{wcs}".supplant(wcsO));
      writeln("");
